...
export async function stake(amount: number): Promise<Result> {
  try {
    ...
    // FIX — this wrote a hardcoded 24.8 regardless of TOKEN.stakingApy in
    // pulse-data.ts, so changing the platform's advertised APY there would
    // silently stop matching what got persisted per staking position.
    await db.from('staking_positions').insert({ user_id: user.id, amount, apy: TOKEN.stakingApy, active: true })
    await recordTxn(user.id, {
      type: 'stake',
      amount,
      currency: 'PULSE',
