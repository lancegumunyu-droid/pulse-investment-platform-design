...
export async function sendToUser(recipientHandle: string, amount: number) {
  const supabase = await getSupabase()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { ok: false as const, error: 'Unauthorized' }
  ...
  try {
    ...
    // Resolve recipient by username, referral code, or wallet id
    const { data: recipient } = await db
      .from('profiles')
      .select('id, username')
      .or(`username.eq.${recipientHandle},referral_code.eq.${recipientHandle}`)
      .maybeSingle()

    if (!recipient) return { ok: false as const, error: 'Recipient not found' }
    if (recipient.id === user.id) return { ok: false as const, error: 'You cannot send funds to yourself' }

    // Deduct from sender, credit recipient. adjustAccount() throws on insufficient balance.
    await adjustAccount(user.id, { cash_balance: -amount })
    await adjustAccount(recipient.id, { cash_balance: amount })

    await recordTxn(user.id, {
      type: 'p2p_send',
      amount,
      currency: 'USD',
      meta: { toUserId: recipient.id, toHandle: recipient.username },
    })
    await recordTxn(recipient.id, {
      type: 'p2p_receive',
      amount,
      currency: 'USD',
